  ...
  methods: {
    ...
    async uploadImg(item) {
      ...
      const reduceResult = (imgItem, res) => {
        ...
        // eslint-disable-next-line
        imgItem.src = this.solveImg(res.path)
        // eslint-disable-next-line
    ...
    async getValue() {
      ...
      const result = imgInfoList.map(item => {
        ...
        const val = {
          ...
          src: this.solveImg(item.src) || null,
          originSrc: this.solveImg(item.src) || null,
    ...
    solveImg(imgUrl) {
      if(!imgUrl) return ''
      imgUrl = imgUrl.replace('https://cms.mgoi.net/uploads/','')
      imgUrl = imgUrl.replace('https://sapi.mgoi.net/uploads/','')
      return imgUrl
    },
