<template>
  <div class="container">
    <div class="title"><span>添加分类</span> <span class="back" @click="back(false)"> <i class="iconfont icon-fanhui"></i> 返回 </span></div>
    <div class="wrap">
      <el-row>
        <el-col :lg="16" :md="20" :sm="24" :xs="24">
          <el-form :model="form" status-icon ref="form" v-loading="loading" :rules="rules" label-width="120px" @submit.native.prevent>
            ...
            <el-form-item label="图片" prop="img">
                <upload-imgs ref="uploadEle" :rules="imgRules" :maxNum="1" />
            </el-form-item>
      ...
      this.$refs[formName].validate(async valid => {
        if(valid) {
          try {
            ...
            this.loading = true
            const res = await culturalCategory.addCate(this.form)
            this.loading = false
            if (res.error_code === 0) {
