	...
	<div>
		...
		<div class="container" v-if="redirectType === 'list'">
			<div class="header">
				<div class="title">
					<span>子分类列表</span>
					<el-button class="add-banner-item" type="primary" plain @click="handleAdd">添加子分类</el-button>
				</div>
  ...
  export default {
    ...
    methods: {
      async handleCellSave({ row, prop }) {
        ...
        this.loading = true
        const res = await culturalCategory.updateSingle(info)
        this.loading = false
        if (res.error_code === 0) {
      ...
      async getCates() {
        ...
        this.loading = true
        try {
          let cateList = await culturalCategory.getSonCate(id)
      ...
      handleDelete({ index, row }) {
        this.$confirm('此操作将永久删除该图书, 是否继续?', '提示', {
          ...
          this.loading = true
          const id = [row.id];
          const res = await culturalCategory.delCate(id)
          this.loading = false
          if (res.error_code === 0) {
            this.getCates()
            this.$message({
