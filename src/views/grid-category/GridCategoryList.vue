	...
	<div>
		...
		<div class="container" v-if="redirectType === 'list'">
			<div class="header">
				<div class="title">
					<span>宫格列表</span>
					<el-button class="add-banner-item" type="primary" plain @click="handleAdd">添加宫格</el-button>
				</div>
			</div>
			<feiy-table
		...
		<grid-category-edit v-else-if="redirectType === 'edit'" :editID="editID" @close="closePage"></grid-category-edit>
		<grid-category-add v-else-if="redirectType === 'add'" @close="closePage"></grid-category-add>
	</div>
</template>

<script>
import gridCategory from '@/models/grid-category'
import GridCategoryAdd from './GridCategoryAdd'
...
export default {
  ...
  methods: {
    async handleCellSave({ row, prop }) {
      ...
      this.loading = true
      const res = await gridCategory.updateSingle(info)
      this.loading = false
      if (res.error_code === 0) {
    ...
    async getCates() {
      try {
        this.loading = true
        const cateList = await gridCategory.getGrids()
        cateList.forEach(val => {
          val.show = !!val.show
        })
        this.tableData = cateList
        this.loading = false
      } catch (error) {
        this.loading = false
        if (error.error_code === 81006) {
          this.tableData = []
        }
      }
    },
