<template>
	<div class="container">
		<div class="title">新建Notice <span class="back" @click="back(false)"> <i class="iconfont icon-fanhui"></i> 返回 </span></div>
		<div class="wrap">
			<el-row>
...
export default {
	...
	methods: {
		...
		async getNotice() {
			this.loading = true
			try {
				this.form = await notice.getNotice(this.editID)
		...
		submitForm(formName) {
			this.$refs[formName].validate(async valid => {
				if(valid) {
					this.loading = true
					try {
						const res = await notice.editNotice(this.editID, this.form)
						if (res.error_code === 0) {
